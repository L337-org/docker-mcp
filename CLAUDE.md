...
- **`compose`** reads its files from a working directory, so `_run_compose` routes through `remote_stage_and_exec`, which stages `project_dir` (or the server's cwd) and runs there. `compose_list` is the exception — it asks the daemon, so it takes the exec-only path. **`compose_cp` is refused outright** under remote-exec: one side of the copy is a path on *this* host, which the remote-exec model cannot be. The refusal points at **`compose_copy`** — the same job (same `SERVICE:PATH` addressing, unpacked files) done through the Engine API, so it needs no CLI on either side and works over `tcp://`+TLS too, where no CLI-based fallback could. `compose_copy` lives in `compose.py` because the module owns the *domain*, not the backend: it is the one SDK-backed tool there, resolving service → container by the labels Compose stamps (`com.docker.compose.project` / `.service` / `.container-number`, skipping `oneoff`) rather than reading a Compose file, so Compose's own project-name normalisation is never reimplemented. It extracts with `filter="data"` explicitly, since Python does not filter by default and a container is not a trusted source of tar member names.
