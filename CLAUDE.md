...
| `docker_mcp/tools/_ssh_proxy.py` | Per-call paramiko proxy (dial-stdio) plus the remote-exec primitives, so CLI-backed tools reach `ssh://` daemons without a system `ssh` binary (private) | — |
...
Consequences to state in the docstring of any tool that gains this path: the command runs as the **remote** SSH user, so registry credentials come from *its* `~/.docker/config.json` (`system_login` talks to the daemon through the SDK and never writes the remote CLI's config); local filesystem inputs are **not** staged; `stdin`/`extra_env` are rejected rather than dropped; and the remote must be POSIX (`uname -s` allow-list — sshd inside WSL is Linux and accepted; a Windows-side cmd/PowerShell sshd, or MSYS/Cygwin on a Windows host, is refused by name). **Wired in so far: `scout` only** — its subcommands take image references and read nothing locally, and the one exception (`scout_compare`'s `to`, which may name a local directory or archive) is refused when the value exists locally instead of being resolved against the remote filesystem.
