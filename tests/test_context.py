...
def test_context_ls_parses_json_lines():
    ...
    with patch("tools.context.run_docker", return_value=_ok(payload)) as run:
        result = context_ls()
    assert result == [
        {
            "Name": "default",
...
def test_context_ls_skips_blank_lines():
    payload = '{"Name":"a"}\n\n{"Name":"b"}\n'
    with patch("tools.context.run_docker", return_value=_ok(payload)):
        result = context_ls()
    assert result == [{"Name": "a"}, {"Name": "b"}]


def test_context_ls_raises_on_failure():
    with patch("tools.context.run_docker", return_value=_fail("permission denied")):
...
def test_context_inspect_raises_on_empty_array():
    with patch("tools.context.run_docker", return_value=_ok("[]")):
...
def test_context_inspect_raises_on_failure():
    with patch("tools.context.run_docker", return_value=_fail("context not found")):
        with pytest.raises(RuntimeError, match="context not found"):
            context_inspect("missing")


def test_context_create_minimal():
    with patch("tools.context.run_docker", return_value=_ok("remote\n")) as run:
...
def test_context_create_with_tls_and_description():
    with patch("tools.context.run_docker", return_value=_ok()) as run:
        context_create(
            "remote",
...
def test_context_create_returns_stderr_on_failure_without_raising():
    ...
    with patch("tools.context.run_docker", return_value=_fail("context already exists")):
...
def test_context_use_invokes_correct_args():
    with patch("tools.context.run_docker", return_value=_ok("Current context: remote\n")) as run:
