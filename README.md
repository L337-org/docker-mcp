...
**No local Docker — or a missing plugin.** The proxy above still needs a local `docker` binary to point at the remote daemon, and Compose, Buildx and Scout each additionally need their CLI plugin. Whenever what a call needs is missing locally — a workstation with only SSH access to real Docker hosts, or an ordinary Engine install without the compose plugin — the docker-py-backed tools (containers, images, networks, volumes, swarm, …) work anyway, and the CLI-backed ones (**Compose, Stack, Buildx, Scout**) fall back to running the command **on the target host itself**, over the same paramiko connection. Local files a command reads are copied there first — a Compose project directory, a bake file, a build context (honouring `.dockerignore`) — into a private `0700` temp directory that is removed when the call returns. Worth knowing:
