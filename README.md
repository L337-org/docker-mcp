...
- **Intel (x86_64) macOS only:** installing natively (via `uvx`/`pip`, or the `.mcpb` bundle, both of
  ...
  `cryptography` — a transitive dependency — has shipped no x86_64 macOS wheel since version 49.0.0
  and must be built from source there. If you'd rather not install a build toolchain, use the
...
- **`cryptography` (a transitive dependency, via `mcp` → `pyjwt[crypto]`) is pinned `>=50` unconditionally, including on Intel macOS, to close a high-severity CVE (a PKCS#7 EnvelopedData decryption Bleichenbacher oracle) rather than staying on a known-vulnerable 48.x to preserve a wheel-only install on that one platform.** No 49.x-or-later release ships an x86_64/universal2 macOS wheel — confirmed permanent, not transient — so a native install there (`uvx`, `pip`, or the `.mcpb` bundle) now builds it from source, needing [Rust](https://rustup.rs/) and OpenSSL 3.x. This project doesn't decrypt PKCS#7 envelopes itself, but a transitive dependency carrying a known-exploitable flaw is not something to ship knowingly. If you'd rather not install a build toolchain, use the [container image](#run-as-a-container) instead (built centrally on Linux, so it never hits this).
