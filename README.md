...
**No local Docker at all.** The proxy above still needs a local `docker` binary to point at the remote daemon. With none installed — a workstation that only has SSH access to real Docker hosts — the docker-py-backed tools (containers, images, networks, volumes, swarm, …) work anyway, and the CLI-backed ones (**Compose, Stack, Buildx, Scout**) fall back to running the command **on the target host itself**, over the same paramiko connection. Local files a command reads are copied there first — a Compose project directory, a bake file, a build context (honouring `.dockerignore`) — into a private `0700` temp directory that is removed when the call returns. Worth knowing:

- **It's a fallback, never a preference.** A working local CLI is always used instead, so nothing changes for a normal install.
