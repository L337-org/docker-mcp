...
- **A few things are refused rather than half-done**: `compose_cp` (its other side is a path on *this* host — use **`compose_copy`**, which does the same job through the Engine API and so needs no CLI on either side), and for `buildx_build` a filesystem `dest=` in `output`/`cache_to`, a local `src=` in `cache_from`, or any `ssh=` — each would resolve on the remote machine.
