...
#
# Named `swarm_*` because their scope is the cluster, not one service: `service_task_list` would
# read as a second, redundant spelling of `service_ps`. They live in this module (domain
# `services`) rather than in `swarm.py` deliberately, so that `DOCKER_MCP_SERVER_DISABLE=services`
# takes them with it -- a task's `Spec` carries the full `ContainerSpec`, so leaving a cluster-wide
# task query registered in the `swarm` domain would let a disabled domain's data out through
# another domain's gate. Prefix-does-not-match-domain has precedent (`host_list` lives in
# `system.py`); see CLAUDE.md "Tool naming convention".
#
