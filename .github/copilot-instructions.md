...
There is a dependencies line in pyproject.toml: `"cryptography>=50"`
This is a deliberately unconditional floor (no platform marker) fixing a high-severity CVE (Dependabot alert #16, a PKCS#7 decryption Bleichenbacher oracle) — do not relax it below 50, and do not reintroduce a platform-scoped cap (e.g. for Intel macOS) that would leave that platform on a vulnerable version while others get the fix.
An earlier `cryptography<49; platform_system == 'Darwin' and platform_machine == 'x86_64'` cap was found to silently hold back *every* platform, not just the scoped one — a lone upper-bound marker doesn't force `uv lock` to fork the resolution, so the whole lockfile stayed on the vulnerable version.
Intel (x86_64) macOS has shipped no `cryptography` wheel since 49.0.0 and now builds it from source (Rust + OpenSSL 3.x required); this is documented in README.md (Requirements and Security considerations) as an accepted trade-off, with the container image offered as a toolchain-free alternative. The weekly canary's wheels-only check (.github/workflows/canary.yaml) is scoped to exempt only cryptography on x86_64-apple-darwin, not `--only-binary :all:` generally.
If a PR touches this line, check it against tests/test_pyproject_pins.py::test_cryptography_floor_is_not_capped_below_the_fix and do not allow it to be merged without a strong justification and approval from the repo owner.
