...
| `_ssh_proxy.py` | Per-call paramiko proxy (dial-stdio) plus remote-exec primitives, so CLI-backed tools reach `ssh://` daemons without a system `ssh` binary (private) | — |
